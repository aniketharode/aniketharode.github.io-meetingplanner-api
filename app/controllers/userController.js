...
apiResponse = response.generate(false,'usser created succcessfully',200,resolve);
res.send(apiResponse);
})
.catch((err) => {
    console.log(err);
...
meetingPlanner=(req,res)=>{
    let apiResponse = response.generate(false, 'Login Successful', 200, req.user);
    res.status(200)
    res.send(apiResponse)
    //console.log("in the meeting planner");
}
